...
void setStyles(void) {
	// selected color
	if(!bDoVS) {
		gnuplot_cmd(gplot, "set style line 1 lt 2 lc rgb '#d05000' lw %d pt 6 ps %f", lineWidth, pointSize);
		gnuplot_cmd(gplot, "set style line 2 lt 2 lc rgb '#d05000' lw %d pt 6 ps %f", lineWidth, pointSize);
	} else {
...
void setScalingRotated(void) {
	...
	// gnuplot_cmd(gplot, "set key default");
}
...
char *readEdataInfo(char *progname) {
	...
	if(FetchRow(res_set)) {
		...
		strcpy(color, FetchField(res_set, "color"));
		...
		gnuplot_cmd(gplot, "set style line 31 lt 2 lc rgb '#%s' lw 1 pt 6 ps 1 ", color);
...
void buildLogdataFile(char *progname, char* tablename) {
	...
	while(FetchRow(res_set)) {
		...
		i++;
		if(bDoVS==0) depth=tvd-(-tan(plotdip/57.29578)*fabs(vs-lastVS))-(lastTVD-lastDepth);
		else depth=vs;
		if(depth<mintvd)	mintvd=depth;
		if(depth>maxtvd)	maxtvd=depth;
		if(vs<minvs)	minvs=vs;
		if(vs>maxvs)	maxvs=vs;
		if(depth==lastDepth) {	// same depth
			continue;
		}
...
int main(int argc, char * argv[])
{
	...
	while(FetchRow(res_set2)) {
		...
		// if(wlid==id && i>0) {	// minimum 2 files
			// dataFNcount+=2;
			// strcpy(dataFN[dataFNcount], tmpnam(NULL));
			// strcpy(dataFN[dataFNcount+1], tmpnam(NULL));
		// }
		...
		// if(wlid==id || i==0)	// a third file if selected id falls in between
		// {
			// dataFNcount+=2;
			// strcpy(dataFN[dataFNcount], tmpnam(NULL));
			// strcpy(dataFN[dataFNcount+1], tmpnam(NULL));
		...
		for(i=0; strlen(dataFN[i]); i+=2) {
			if(dataCnt[i]>1) {
				...
				if(wlid!=dataID[i]) {
					if(dataID[i]!=-9) {
						// printf("Plot data -9\n");
						sprintf(plotstr, " '%s' with lines ls 3 t 'Gamma'", dataFN[i]);
					} else {
						// printf("Plot data\n");
						sprintf(plotstr, " '%s' with lines ls 31 t '%s'", dataFN[i], edataLabel);
					}
				}
				else {
