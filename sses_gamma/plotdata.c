...
typedef struct t_edata T_EDATA;
struct t_edata{
	char label[4095];
	char color[128];
	char table[4095];
	...
	float plotbias;
	float plotscale;
	char plota_filename[L_tmpnam];
	FILE *plotaFile;
	char plotb_filename[L_tmpnam];
	FILE *plotbFile;

};
#define MAX_EDATA 4
T_EDATA	edatas[MAX_EDATA];
...
void setStyles(void) {
	...
	// TOT and BOT lines
	gnuplot_cmd(gplot, "set style line 31 lt 2 lc rgb '#d00070' lw 4 pt 6 ps 1 ");
...
void setScaling(void) {
	...
	gnuplot_cmd(gplot, "set terminal png size %.0f,%.0f font arial %d", plotWidth, plotHeight, fontSize);
	if(rightScale<=0) {
		maxVal=ceil(maxVal);
...
void setScalingRotated(void) {
	...
	gnuplot_cmd(gplot,"set xzeroaxis linetype -1 linewidth 2.5");

	//gnuplot_cmd(gplot, "set terminal png size %.0f,%.0f font arial %d", plotWidth, plotHeight, fontSize);
	sprintf(tmpstr,"set terminal pngcairo size %.0f,%.0f font 'arial,%d'", round(plotWidth), round(plotHeight), fontSize);
	gnuplot_cmd(gplot,"%s",tmpstr);
	printf("sses_gamma: %s\n",tmpstr);
	printf("sses_gamma: right scale:%f maxVal:%f\n", rightScale2,maxVal);
	if(rightScale<1.0) {
		maxVal=ceil(maxVal);
		rightScale=maxVal-((int)maxVal%10)+10;
		if(rightScale<1.0)	rightScale=1.0;
	...
	if(bDoVS) {
		if(!bUseLogScale){
			...
			gnuplot_cmd(gplot,
						"set label 1 \"%s\" at %f, %f, 0 left front textcolor rgb '#%s' offset character 1, -.7, 0",
						"Resistivity", startDepth, rightScale, lineColor);
		}
		int lblcnt = 2;
		int char_offset= -1;
		for(i=0;edatas[i].line!=0;i++){
			if(singlePlot==0){
				...
				gnuplot_cmd(gplot,
							"set label %i \"%s(%.1f)\" at %f, %f, 0 left front textcolor rgb '#%s' offset character 1, -.7, 0",
...
void readAppinfo(char *progname) {
	...
	if(FetchRow(res_set)) {
		...
		nosgta = atoi(FetchField(res_set,"sgta_off"));
		if(!bForceRotate)
...
void readEdataInfo(char *progname) {
	...
	while(FetchRow(res_set)) {
		strcpy(edatas[i].table, FetchField(res_set, "tablename"));
		strcpy(edatas[i].color, FetchField(res_set, "color"));
		strcpy(edatas[i].label, FetchField(res_set, "label"));
		edatas[i].leftScale2=atof(FetchField(res_set, "scalelo"));
		edatas[i].rightScale2=atof(FetchField(res_set, "scalehi"));
		edatas[i].line = stylecnt;
		gnuplot_cmd(gplot, "set style line %i lt 2 lc rgb '#%s' lw 1 pt 6 ps 1 ",stylecnt, edatas[i].color);
		if(rightScale>0 && edatas[i].rightScale2>0) {
			edatas[i].plotbias=edatas[i].leftScale2;
			edatas[i].plotscale=rightScale/edatas[i].rightScale2;
		}
		i++;
		stylecnt++;
	}
...
void buildLogdataFile(char *progname, char* tablename,T_EDATA edata_in) {
	FILE *out1=NULL;
	FILE *out2=NULL;
	int i;
	float value;
	...
	if(edata_in.line==0){
		...
		pscale = edata_in.plotscale;
		pbias = edata_in.plotbias;
	}
...
int main(int argc, char * argv[])
{
	...
	printf("Start MD:%.2f End MD:%.2f Depth Range:%.2f Data Set Count:%d\n",startmd,endmd,depthRange,datasetCount);
	...
	for(i=0;i<MAX_EDATA;i++){
		edatas[i].line=0;
		edatas[i].plotscale=plotscale;
		edatas[i].plotbias =plotbias;
	}
	int did_loop=0;
	while(FetchRow(res_set2)) {
		...
		buildLogdataFile(argv[0], whatToPlot,edatas[2]);
	...
	if(did_loop<=0 && singlePlot==0){
		...
		dataID[dataFNcount]=-6;

		dataID[dataFNcount+1]=-6;
	...
	if(bDoVS) {
		...
		for(i=0;edatas[i].line!=0;i++){
			strcpy(edatas[i].plota_filename,tmpnam(NULL));
			strcpy(edatas[i].plotb_filename,tmpnam(NULL));
			strcpy(dataFN[dataFNcount],edatas[i].plota_filename);
			strcpy(dataFN[dataFNcount+1], edatas[i].plotb_filename);
			buildLogdataFile(argv[0],edatas[i].table,edatas[i]);
	...
	// start building the plot command
	// include the rotate slide file if it exists

	sprintf(datfn,"/tmp/%s_rotslide.dat",dbname);
	printf("datfn=%s\n",datfn);
	if(stat(datfn,&mybuf) == 0)
	{
		printf("Yes!, %s exists\n",datfn);
		sprintf(cmdstr,"plot '%s' with filledcurve notitle lc rgb '#aaaaff' fs transparent solid 0.75 noborder,",datfn);
		//sprintf(cmdstr,"plot '%s' with lines,",datfn);
	}
	else strcpy(cmdstr, "plot");

	int j=0;
	int hasplot=0;
	for(i=0; strlen(dataFN[i]); i+=2) {
		if(dataCnt[i]>1) {
			if(i>0 && hasplot>0)	strcat(cmdstr, ",");
			if(wlid!=dataID[i]) {
				if(dataID[i]!=-9) {
					...
					// else
						int line = edatas[j].line;
						sprintf(plotstr, " '%s' with lines ls %i t ''", dataFN[i],line);
						j++;
						hasplot=1;
				}
			}
