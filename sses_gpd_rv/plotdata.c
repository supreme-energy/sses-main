...
void setupPage(void) {
	...
	gnuplot_cmd(gplot, "set style line 1 lt 2 lc rgb '#7070ff' lw %d pt 6 ps %f", 2, pointSize);
	...
	// selected color
	gnuplot_cmd(gplot, "set style line 3 lt 2 lc rgb 'red' lw %d pt 6 ps %f", 2, 0);
	...
	// grid colors
	gnuplot_cmd(gplot, "set style line 20 lt 2 lc rgb 'black' lw %d ", 3);
...
void setScaling(void) {
	...
	if(!bNoDepth) {
		...
		if(norightmargin==0){
			gnuplot_cmd(gplot, "set rmargin 6");
		} else {
	...
	sprintf(str, "  offset character 46 font '/usr/share/fonts/truetype/arial.ttf,%d'", fontSize);
	//sprintf(str2, "  offset character -1 font '/usr/share/fonts/truetype/arial.ttf,%d'", fontSize);
...
int main(int argc, char * argv[])
{
	...
	if(bDoWellLog) {
		...
		bias=0.0;
		scale=1.0;
		sprintf(cmdstr, "select * from edatalogs where label='ROP'");
			if (DoQuery(res_set2, cmdstr)) {
				//fprintf(stderr, "%s: Error in select query for table %s\n", progname, tablename);
				FreeResult(res_set2);
				return;
				...
				while(FetchRow(res_set2)){
					buildLogdataFile(argv[0], FetchField(res_set2,"tablename"), 0, "ASC");
				}
			}
	}
	...
	printf("Fancy plot\n");
		...
		if(plamd==2){
			if (DoQuery(res_set2, "select md,tvd,inc,azm from surveys where plan=0")) {
						fprintf(stderr, "%s: Error in select query for table %s\n",
							argv[0], cmdstr);
						FreeResult(res_set2);
						CloseDb();
						...
						while(FetchRow(res_set2)) {
							float ltvd = atof(FetchField(res_set2, "tvd"));
							gnuplot_cmd(gplot,"set arrow from %f,%f to %f,%f nohead lc rgb '%s' lw 1 front",0.0,atof(FetchField(res_set2, "md")),(float)rightScale-1.0,atof(FetchField(res_set2, "md")),"black");
							sprintf(plotstr, "set label 'TVD:%.02f INC:%.02f AZM:%.02f MD:%.02f' at %f,%f front tc rgb 'black' offset character %f,-0.5 font '/usr/share/fonts/truetype/arial.ttf,%d\n",
									ltvd,
									atof(FetchField(res_set2, "inc")),
									atof(FetchField(res_set2, "azm")),
									atof(FetchField(res_set2, "md")),0.0,atof(FetchField(res_set2, "md")),1.0,6);
							gnuplot_cmd(gplot,plotstr);
