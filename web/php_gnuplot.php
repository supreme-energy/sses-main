...
// $tempDir = 'C:\temp'; // somewhere we can store the temporary data files
$tempDir = 'tmp';
...
class GNUPlot {
    ...
    {
       $this->exe( "set arrow from $x1,$y1 to $x2,$y2 nohead ls 2\n" );
    }
    ...
    function export( $pic_filename ) {
        ...
        if (preg_match("/\.png$/", $pic_filename)) {
			// $this->exe("set term png\n");
			$this->exe("set term png font arial 8 size $this->width,$this->height\n");
		}
        elseif (preg_match("/\.e?ps$/", $pic_filename)) $this->exe( "set term postscript enhanced color \"Helvetica\" 10\n");
