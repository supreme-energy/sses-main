...
class PDF extends FPDF
{
	...
	function ReportHeader($now)
	{
		...
		$this->Cell(10.2, .1, "Copyright 2010-2011 Digital Oil Tools", 0, 1, 'R', false);
		$this->SetFontSize(7);
		$this->Ln();
	...
	function ReportAnnos(){
		...
		if(count($annos)){
			...
			$loopmod=0;
			if(count($additionlgraphs)>0){
				$loopmod=2+(count($additionlgraphs)*3);
			}
			while($i<(37-$loopmod-(count($annos)+$this->projection_count))){
				$this->Ln(); $this->hdrheight+=$h;
				$i++;
			}
			$this->SetFillColor(110, 174, 126);
