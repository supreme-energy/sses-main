<?
	...
	class PolarisConnection extends WitsmlData {
		...
		function prepare_las_data($sdepth=0,$edepth=100,$pass=1,$include_additional=false,$dip=false,$fault=false){
			$body='<log uidWell="'.$this->well.'" uidWellbore="'.$this->wellbore.'" uid="'.$this->logid.'">' .
						'<name/>' .
						'<startIndex>'.$sdepth.'</startIndex>' .
						'<endIndex>'.$edepth.'</endIndex>' .
						'<logCurveInfo uid=""><columnIndex></columnIndex>' .					
						'</logCurveInfo>' .
						'<logData><data/></logData></log>';
			...
			$this->db->OpenDb();
			$headersmnemo=array();
			foreach($xmlout->log->logCurveInfo as $column){
				$edata = array();
			...
			if(!$welllogexists){
			    ...
			    if($this->db->FetchRow()){
					...
					$tablename="wld_$id";
					$real="Polaris Auto Import $sdepth - $edepth";
...
//					if($this->db->FetchRow())
//					{
//						$autoapply = $this->db->FetchField('cvalue');
//						if($autoapply == 'Manual')
//						{
...
//					if($dip != '0')
//					{
//						$this->db->DoQuery("UPDATE welllogs SET fault='0',dip='$dip' WHERE id='$id';");
//					}
   ...
   function get_polaris_survey(){
   		$body = '<trajectory uidWell="'.$this->uidWell.'" uidWellbore="'.$this->uidWellBore.'"><name/><trajectoryStation uid="">' .
				'<md/>' .
				'<incl/>' .
				'<azi/>' .
				'<vertSect/>' .
