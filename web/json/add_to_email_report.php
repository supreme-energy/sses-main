<?php
	...
	if($reportv=="las"){
		...
		$db->DoQuery("update emaillist set las_file=1 where id=$id");
	}else if($reportv=="r1"){
		$addtolist =  $db->FetchField("report_1");
		$db->DoQuery("update emaillist set report_1=1 where id=$id");
	}else if($reportv=="r2"){
		$addtolist =  $db->FetchField("report_2");
		$db->DoQuery("update emaillist set report_2=1 where id=$id");
	}
	if($addtolist==1){
		$rval = "'already_added'";
	} else {
